'use client';

import Link from 'next/link';
import { UseFormRegister, FieldErrors } from 'react-hook-form';
import { Input } from '@/src/shared/ui/components/Input';
import { Button } from '@/src/shared/ui/components/Button';
import { Card } from '@/src/shared/ui/components/Card';
import { Textarea } from '@/src/shared/ui/components/Textarea';
import { Select } from '@/src/shared/ui/components/Select';
import { MustahiqFormValues } from '../../domain/mustahiq.types';
import { MUSTAHIQ_STATUS } from '@/src/shared/config/constants';
import { Asnaf } from '@/src/shared/types/common.types';

interface MustahiqFormProps {
    register: UseFormRegister<MustahiqFormValues>;
    errors: FieldErrors<MustahiqFormValues>;
    onSubmit: () => void;
    isLoading: boolean;
    error: string | null;
    asnafList: Asnaf[];
    isEditMode?: boolean;
}

export const MustahiqForm = ({
    register,
    errors,
    onSubmit,
    isLoading,
    error,
    asnafList,
    isEditMode = false
}: MustahiqFormProps) => {

    const statusOptions = [
        { value: MUSTAHIQ_STATUS.ACTIVE, label: 'Aktif' },
        { value: MUSTAHIQ_STATUS.INACTIVE, label: 'Tidak Aktif' },
        { value: MUSTAHIQ_STATUS.PENDING, label: 'Pending' },
    ];

    const asnafOptions = [
        { value: '', label: 'Pilih Asnaf' },
        ...asnafList.map(asnaf => ({
            value: asnaf.id,
            label: asnaf.name
        }))
    ];

    return (
        <Card className="p-6">
            <form onSubmit={onSubmit} className="space-y-6">
                <div className="border-b border-gray-100 dark:border-dark-border pb-4 mb-4">
                    <h3 className="text-lg font-semibold text-gray-900 dark:text-text-primary">
                        {isEditMode ? 'Edit Data Mustahiq' : 'Tambah Mustahiq Baru'}
                    </h3>
                    <p className="text-sm text-gray-500 dark:text-gray-400">
                        {isEditMode
                            ? 'Perbarui informasi detail mengenai mustahiq ini.'
                            : 'Lengkapi formulir di bawah untuk menambahkan mustahiq baru.'}
                    </p>
                </div>

                <div className="space-y-4">
                    <Input
                        label="Nama Mustahiq"
                        placeholder="Masukkan nama lengkap"
                        error={errors.name}
                        registration={register('name')}
                    />

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <Input
                            label="Nomor Telepon"
                            placeholder="Contoh: 08123456789"
                            error={errors.phoneNumber}
                            registration={register('phoneNumber')}
                        />

                        <Select
                            label="Asnaf"
                            options={asnafOptions}
                            error={errors.asnafID}
                            registration={register('asnafID')}
                        />
                    </div>

                    <Select
                        label="Status"
                        options={statusOptions}
                        error={errors.status}
                        registration={register('status')}
                    />

                    <Textarea
                        label="Alamat"
                        placeholder="Masukkan alamat lengkap"
                        registration={register('address')}
                        error={errors.address}
                        rows={3}
                    />

                    <Textarea
                        label="Deskripsi"
                        placeholder="Masukkan deskripsi tambahan (opsional)"
                        registration={register('description')}
                        error={errors.description}
                        rows={3}
                    />
                </div>

                {error && (
                    <div className="p-3 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 rounded-lg text-sm border border-red-100 dark:border-red-800">
                        {error}
                    </div>
                )}

                <div className="flex items-center justify-end gap-3 pt-4 border-t border-gray-100 dark:border-dark-border mt-6">
                    <Link href="/mustahiq">
                        <Button variant="outline" type="button" disabled={isLoading}>
                            Batal
                        </Button>
                    </Link>
                    <Button type="submit" isLoading={isLoading}>
                        {isEditMode ? 'Simpan Perubahan' : 'Simpan Mustahiq'}
                    </Button>
                </div>
            </form>
        </Card>
    );
};